...
final class YouTubeBackgroundRepository {
    ...
    private final SharedPreferences selections;
    ...
    YouTubeBackgroundRepository(Context context) {
        ...
        selections = appContext.getSharedPreferences("youtube_background_selections", Context.MODE_PRIVATE);
    ...
    void load(
        ...
        VideoInfo selected = selectedVideo(track.stableKey());
        if (selected != null) {
            notifyLoaded(callback, safeRequestKey, selected, true);
            return;
        }
    ...
    VideoInfo selectedVideo(String trackKey) {
        try {
            VideoInfo info = VideoInfo.fromJson("", new JSONObject(selections.getString(trackKey, "")));
            return info == null || info.youtubeVideoId.isEmpty() ? null : info;
        } catch (Exception ignored) { return null; }
    }

    void selectVideo(String trackKey, VideoInfo info) {
        if (trackKey == null || trackKey.isEmpty()) return;
        if (info == null) { selections.edit().remove(trackKey).apply(); return; }
        if (info.youtubeVideoId.isEmpty()) return;
        try { selections.edit().putString(trackKey, info.toJson().toString()).apply(); }
        catch (Exception ignored) { }
    }

    void loadCommunityVideos(String isrc, TrackSnapshot track,
            java.util.function.BiConsumer<List<VideoInfo>, String> callback) {
        executor.execute(() -> {
            try {
                if (TrackSnapshot.normalizeIsrc(isrc).isEmpty()) throw new IOException("Missing ISRC");
                String url = Uri.parse("https://ivlis.kr/ivLyrics/openvideo/community").buildUpon()
                        .appendQueryParameter("isrc", isrc)
                        .appendQueryParameter("trackId", track.trackId)
                ...
                if (!response.optBoolean("success")) throw new IOException("Video lookup failed");
                JSONObject data = response.optJSONObject("data");
                JSONArray videos = data == null ? null : data.optJSONArray("videos");
                List<VideoInfo> choices = new ArrayList<>();
                java.util.Set<String> ids = new java.util.HashSet<>();
                for (int index = 0; videos != null && index < videos.length(); index++) {
                    VideoInfo info = VideoInfo.fromJson(isrc, videos.optJSONObject(index));
                    if (info != null && !info.youtubeVideoId.isEmpty() && ids.add(info.youtubeVideoId)) choices.add(info);
                }
                mainHandler.post(() -> callback.accept(choices, null));
            } catch (Exception error) {
                mainHandler.post(() -> callback.accept(Collections.emptyList(), "Video lookup failed"));
            }
        });
    }
    ...
    static final class VideoInfo {
        ...
        VideoInfo(
            ...
            this.captionStartTimeSeconds = hasCaptionStartTime && Double.isFinite(captionStartTimeSeconds) ? Math.max(0d, captionStartTimeSeconds) : 0d;
        ...
        static VideoInfo fromJson(String fallbackIsrc, JSONObject object) {
            ...
            String captionKey = object.has("captionStartTime") ? "captionStartTime" : "startTime";
            boolean hasCaption = object.has(captionKey) && !object.isNull(captionKey);
            return new VideoInfo(
                    ...
                    hasCaption,
                    hasCaption ? object.optDouble(captionKey, 0d) : 0d,
